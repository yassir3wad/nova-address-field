    ...
    <template slot="field">
      <div class="flex" v-if="manualFill">
        ...
        <div class="w-1/5 p-1">
          <button
            type="button"
            v-if="manualFill"
            v-on:click="fillFields"
            :disabled="!hasUnfilledChanges"
            class="btn btn-default btn-primary hover:bg-primary-dark w-full"
          >{{manualFill}}</button>
        </div>
      </div>
      <vue-google-autocomplete
      ...
      <div v-if="!field.hideToggles" class="flex w-full pt-2">
        <div class="flex w-1/2">
          <checkbox :checked="field.withMap" @input="toggleMap" class="py-2 pr-2"/>
          <label @click="toggleMap" class="inline-block text-80 pt-2 leading-tight">Show Map</label>
        </div>

        <div class="flex w-1/2">
          <checkbox :checked="field.withLatLng" @input="toggleLatLng" class="py-2 pr-2"/>
          <label
            @click="toggleLatLng"
            class="inline-block text-80 pt-2 leading-tight"
          >Show Coordinations</label>
        </div>
      </div>
...
export default {
  ...
  data: function() {
    return {
      ...
      mapOptions: {
        center: new google.maps.LatLng(40.73061, -98.935242),
        zoom: 5
      },
      address: "",
      addressData: {
      ...
      administrative_area_level_1:
        this.field.administrative_area_level_1 || false,
      ...
      address_line_1: this.field.address_line_1 || false,
      relatedValues: {},
      relatedWatchers: [],
    ...
    if (this.field.do_not_store) {
      this.$parent.$children.forEach(component => {
        if (
          component.field &&
          [
            this.field.latitude_field,
            this.field.longitude_field,
            this.field.address_field
  ...
  methods: {
    ...
    getAddressData: function(addressData, placeResultData, id) {
      ...
      this.addressData.countryCode = this.getAddressComponent(
        placeResultData.address_components,
        "country",
        true
      );
      this.addressData.address_line_1 = placeResultData.name;
    ...
    relatedAreEmpty: function() {
      if (
        this.field.address_field &&
        this.relatedValues[this.field.address_field]
      ) {
        if (this.field.address_field_array_key) {
          if (
            this.relatedValues[this.field.address_field][
              this.field.address_field_array_key
            ]
          ) {
            return false;
          }
      ...
      if (
        this.field.latitude_field &&
        this.relatedValues[this.field.latitude_field]
      ) {
        return false;
      }
      if (
        this.field.longitude_field &&
        this.relatedValues[this.field.longitude_field]
      ) {
        return false;
      }
      return true;
    ...
    initializeAddress: function() {
      ...
      if (
        ...
        this.addressData.latitude = this.relatedValues[
          this.field.latitude_field
        ];
        this.refreshMap();
      }
      if (
        ...
        this.addressData.longitude = this.relatedValues[
          this.field.longitude_field
        ];
        this.refreshMap();
      }
      ...
      this.geocoder.geocode({ location: latLng }, function(results, status) {
        if (status === "OK") {
          if (results[0]) {
            ...
            _this.addressData.address_line_1 = results[0].name;

            _this.addressData.countryCode = _this.getAddressComponent(
              results[0].address_components,
              "country",
              true
            );
            _this.addressData.country = _this.getAddressComponent(
              results[0].address_components,
              "country"
            );
            _this.addressData.administrative_area_level_1 = _this.getAddressComponent(
              results[0].address_components,
              "administrative_area_level_1"
            );
            _this.addressData.locality = _this.getAddressComponent(
              results[0].address_components,
              "locality"
            );
            _this.addressData.postal_code = _this.getAddressComponent(
              results[0].address_components,
              "postal_code"
            );
    ...
    updateFields(addressData) {
      ...
      this.$nextTick(() => {
        ...
        Nova.$emit(
          this.field.address_line_1 + "-value",
          addressData.address_line_1
        );
        Nova.$emit(
          this.field.administrative_area_level_1 + "-value",
          addressData.administrative_area_level_1
        );
        var name = addressData.formatted_address;
        if (this.field.address_field_array_key) {
          name = {};
          name[this.field.address_field_array_key] =
    ...
    updateGeoLocationFields(addressData) {
      this.$nextTick(() => {
        ...
        Nova.$emit(
          this.field.longitude_field + "-value",
  ...
  watch: {
    addressData: {
      handler: function(newAddressData) {
        ...
        if (
          !this.addressIsInitializing &&
          (!this.manualFill || this.relatedAreEmpty())
        ) {
